...
class NNStructure:
    ...
    def build_tree(self):
        vecs = [np.r_[np.asarray(n.get_final_state().vector_weighted()), n.get_extension_failure_ratio()]
                for n in self.nodes]
    ...
    def get_nearest_waiting_node(self, s: State) -> Tuple[float, Optional[Node]]:
        ...
        query_vec = np.r_[s.vector_weighted(), 0]
    ...
    def get_nearest_node_tree(self, s: State) -> Tuple[float, Optional[Node]]:
        ...
        query_vec = np.r_[s.vector_weighted(), 0]
        if len(self.nodes) == 1:
            vec = np.r_[self.nodes[0].get_final_state().vector_weighted(), self.nodes[0].get_extension_failure_ratio()]
